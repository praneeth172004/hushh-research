...
| World model/index/registry coherence | `consent-protocol/hushh_mcp/services/world_model_service.py`, `consent-protocol/hushh_mcp/services/domain_registry_service.py` | canonical summary counts + aligned `available_domains` + registry rows | context count drift, domains in blob not in index/registry | run reconciliation path and verify with `audit-world-model-user.mjs` |
...
- World model summary normalization: never roll back sanitization; rollback by adding compatibility read logic, not by storing sensitive summary fields.
